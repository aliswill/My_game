...
public class NPCGambler implements NPCrole {
	...
	public boolean playGuessNumber() {
		...
		int time=0;
		SpeakUtil.speak("穿西裝的男人:答案已經出好，你開始猜吧");	
		Scanner sc = new Scanner(System.in);
		while(time<3) {					
			String v = sc.next();
			while(!CheckType.isNumeric(v)) {
				SpeakUtil.speak("穿西裝的男人:喂喂，我說猜數字欸數字");	
				v = sc.next();
			}
			int guess_num = Integer.parseInt(v);										
			if(guess_num<ans) {
				SpeakUtil.speak("穿西裝的男人:哈哈!再大一些");
			}else if(guess_num>ans) {
				SpeakUtil.speak("穿西裝的男人:可惜囉!再小一些");
			}else {
				SpeakUtil.speak("穿西裝的男人:喔!不!你怎麼可能猜中..");
				
				return true;
			}
			time++;
		}
		SpeakUtil.speak("穿西裝的男人:哈哈!又是一頭肥羊!謝謝你啦~");
		return false;
	}
}
