...
public class NPCTraveler implements NPCrole{
	...
	public void appear() {
		...
		if(v.equalsIgnoreCase("w")) {
			...
			SpeakUtil.speak("狼狽不堪的旅行者瞪了一眼"+hero.getName()+"並踱著緩步走了");
		}else if(v.equalsIgnoreCase("e")) {
			SpeakUtil.speak("狼狽不堪的旅行者：謝謝你啊~你也是旅行者吧?");
			
			SpeakUtil.speak("狼狽不堪的旅行者：其實啊..我剛剛從"+ maputil.getDanagerousDirection(hero)+"方而來");
			
			SpeakUtil.speak("狼狽不堪的旅行者：那裏的敵人都特別強大，你最好不要往那邊走");
			
			SpeakUtil.speak("狼狽不堪的旅行者：那麼再會了，謝謝你的茶水~");
		}else {
			SpeakUtil.speak("("+hero.getName()+"無視了他)");
