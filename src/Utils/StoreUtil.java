...
//	}
	...
	public String welcomStore() {
	SpeakUtil.speak(2,"這裡有熱鬧的商店街，想要拜訪商店嗎? (請輸入Y/N)");
	...
	public void useStore(Hero hero,String welcome_yn) {
		...
		if(welcome_yn.equalsIgnoreCase("Y")) {//開始購物
			...
			if(v1.equalsIgnoreCase("Z")) {
				...
				buy(hero,c);
				SpeakUtil.speak(2,"店主：謝謝光臨!是否繼續購物? (請輸入Y/N)");
				String v2 = sc.next();
				useStore(hero,v2);
			}
			
		}else if(welcome_yn.equals("N")) {
			SpeakUtil.speak(1,"店主：有機會再來哦！");
			//do nothing
		}
	}
	
	public void useHotel(Hero hero) {
		SpeakUtil.speak(2,"今晚需要住宿旅店嗎? (請輸入Y/N)");
		Scanner sc = new Scanner(System.in);
		String v = sc.next();
		while(!v.equalsIgnoreCase("Y")&&!v.equalsIgnoreCase("N")) {
			SpeakUtil.speak(2,"(請輸入Y/N)");
		...
		if(v.equalsIgnoreCase("Y")) {//開始購物
			...
			while(!v1.equalsIgnoreCase("Q")&&!v1.equalsIgnoreCase("W")&&!v1.equalsIgnoreCase("E")&&!v1.equalsIgnoreCase("Z")) {
				SpeakUtil.speak(2,"(按對應鍵入住，或按Z離開)");
				v1 = sc.next();
			}
			relax(v1,hero);
	...
	private void relax(String input,Hero hero) {
		if(input.equalsIgnoreCase("Q")) {
			if(hero.getMoney()>=5) {
				...
				SpeakUtil.speak(1,hero.getName()+"在歇腳的旅店休息了一晚，生命值恢復為"+final_value);
				hero.setLife(final_value);
			}else {
				SpeakUtil.speak(1,"沒錢還是自己搭個帳篷就好吧..");
			}
			...
			if(hero.getMoney()>=10) {
				...
				SpeakUtil.speak(1,hero.getName()+"在闊氣的旅店舒服地休息了一晚，生命值恢復為"+final_value);
				hero.setLife(final_value);
			}else {
				SpeakUtil.speak(1,"沒錢還是自己搭個帳篷就好吧..");
			}
			...
			if(hero.getMoney()>=10) {
				...
				SpeakUtil.speak(1,hero.getName()+"在溫泉會館做了紓壓按摩，生命值恢復了"+l_final_value+",魔法力恢復了"+m_final_value);
